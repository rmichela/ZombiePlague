...
public class ZombieDamage {
    ...
    public ZombieDamage(final Plugin plugin) {
        ...
        plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new DamageApplier() {
            @Override
            public Boolean ShouldApply(int damage) {
                return damage == 1;
            }
        }, 0, (plugin.getConfig().getInt("damageCycleSeconds") * 20));
        plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new DamageApplier() {
            @Override
            public Boolean ShouldApply(int damage) {
                return damage == 2;
            }
        }, 0, (plugin.getConfig().getInt("damageCycleSeconds") * 20) / 2);
        plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new DamageApplier() {
            @Override
            public Boolean ShouldApply(int damage) {
                return damage == 3;
            }
        }, 0, (plugin.getConfig().getInt("damageCycleSeconds") * 20) / 3);
        plugin.getServer().getScheduler().scheduleSyncRepeatingTask(plugin, new DamageApplier() {
            @Override
            public Boolean ShouldApply(int damage) {
                return damage >= 4;
            }
        }, 0, (plugin.getConfig().getInt("damageCycleSeconds") * 20) / 4);
    }
